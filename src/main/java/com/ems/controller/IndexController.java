...
public class IndexController 
{
	...
	public String index(ModelMap map, HttpServletRequest request, Principal principal)
	{
		...
		System.out.println("from index page of index controller" + Roles.ROLE_ADMIN);
...
//			return "redirect:userHome";
	...
	public String getLogedIn(ModelMap map, HttpServletRequest request, Principal principal, TimeZone timeZone)
	{
		...
		if(reg != null)
		{
			...
			UserSetting us = userSettingService.getUserSetting(principal.getName());
			if(us != null)
			{
	...
	@RequestMapping(value = "/changeTimezone", method = RequestMethod.GET)
	@ResponseBody
	public String changeTimezone(ModelMap map, HttpServletRequest request, Principal principal)
	{
		JSONObject obj = new JSONObject();
		...
		{
			request.getSession(true).setAttribute("timezone", TimeZone.getTimeZone(tz));
			obj.put("success", true);
			return obj.toJSONString();
		}
		obj.put("success", false);
	...
	@ResponseBody
	public String getCurrentTime(ModelMap map, HttpServletRequest request, Principal principal) throws ParseException
	{
		TimeZone tz = (TimeZone)request.getSession().getAttribute("timezone");
		JSONObject obj = new JSONObject();
		if(tz != null)
		{
//			long millis = d.getDate(tz).getTime();
//			System.out.println("millis : " +new Date());
//			System.out.println("millis : " +new Date(millis));
			obj.put("millis", new DateFormats().getDate(tz).getTime());
		}
		return obj.toJSONString();
	}
